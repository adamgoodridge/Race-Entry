...
- `DeclarationControllerIT` — PDF returned, correct Content-Type ✅ **DONE** (3 tests)
...
- Slice #10 Email Notifications ✅ **DONE** — `EntryNotificationService` (`au.com.voc.raceEntry.notification`): resolves boat owner email via Boat→Person→User chain; sends plain-text emails for approve (notifyApproved), request-changes (notifyChangesRequested), and cancel (notifyCancelled); `MailException` is caught and logged so notification failures never roll back the main transaction. `spring-boot-starter-mail` added to pom.xml. `EntryService.approve/requestChanges/cancel` each call the notification service after persisting. `EventCancellationMediator` and `EventBoatClassRemovalMediator` call `notifyCancelled` in their cascade loops. All 7 IT test classes gain `@MockBean JavaMailSender` so no real SMTP connection is attempted. Total: 77/77 PASSED.
- Slice #11 Declaration Form PDF ✅ **DONE** — `DeclarationFormService` (`au.com.voc.raceEntry.declaration`): resolves entry, boat, event, boatClass, and drivers; generates a plain-text iText 5 PDF listing event name/start date, boat name/race number/class, and a drivers table (name, SBA licence, SBA expiry) with a signature line. `DeclarationController` adds `GET /api/entries/{id}/declaration` returning `application/pdf` with owner-or-admin check (same pattern as EntryController). `DeclarationControllerIT`: 3 tests — PDF returned for owner (200+Content-Type), PDF returned for admin (200), 403 for non-owner member. Total: 80/80 PASSED.
